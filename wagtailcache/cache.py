...
class FetchFromCacheMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: WSGIRequest) -> Optional[HttpResponse]:
        return process_fetch(request, self._wagcache)


def process_fetch(request: WSGIRequest, cache) -> Optional[HttpResponse]:
    ...
    try:
        cache_key = _get_cache_key(request, cache)
        ...
        response: HttpResponse = cache.get(cache_key)
    ...
    if (
        ...
        not_modified.headers["Cache-Control"] = response.headers[
            "Cache-Control"]
        return not_modified
    return response
    ...
    if timeout != 0:
        try:
            cache_key = _learn_cache_key(
                request, response, timeout, cache
            )
...
def cache_page(view_func: Callable[..., HttpResponse]):
    ...
    def _wrapped_view_func(
        ...
        response = process_fetch(request, wagcache)
        if response is None:
            # Since we don't have a response at this point, process the request.
            response = view_func(request, *args, **kwargs)
        # Cache the response.
        response = process_update(request, response, wagcache)
        return response
